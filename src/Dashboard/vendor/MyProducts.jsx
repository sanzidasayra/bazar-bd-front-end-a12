...
const MyProducts = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/products/vendor?email=${user.email}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="p-4">
      <h2 className="text-3xl font-bold mb-6 text-[#03373D] text-center">My Products</h2>
        ...
        <table className="min-w-full text-sm text-left border border-gray-200 rounded-xl overflow-hidden">
          ...
          <tbody>
            {products.map((product, idx) => (
              ...
              >
                <td className="p-3 font-medium text-gray-800">{product.itemName}</td>
                <td className="p-3 text-gray-700">৳{product.pricePerUnit}/kg</td>
                ...
                <td className="p-3">
                  ...
                  {product.status === "rejected" && (
                    <div className="p-2 bg-red-50 border-l-4 border-red-400 rounded mt-2 text-sm text-red-800">
                      ...
                      <p><strong>Feedback:</strong> {product.rejectionFeedback}</p>
                    </div>
                  )}
                </td>
