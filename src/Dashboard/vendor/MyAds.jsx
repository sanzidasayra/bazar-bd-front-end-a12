...
const MyAds = () => {
  ...
  const fetchAds = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/advertisements?email=${user.email}`);
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`http://localhost:5000/advertisements/${adToDelete._id}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <h2 className="text-3xl font-semibold mb-6 text-gray-800">My Advertisements</h2>
