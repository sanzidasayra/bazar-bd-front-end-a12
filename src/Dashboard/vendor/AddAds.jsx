    ...
    <div className="max-w-2xl mx-auto p-8 bg-white shadow-2xl rounded-xl">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex-1 space-y-5">
          <div>
            <label className="block text-sm font-semibold mb-1 text-[#52616B]">Advertisement Title</label>
            <input
              ...
              maxLength={50}
            />
            {errors.adTitle && <span className="text-red-500 text-xs">Title is required (min 4 characters)</span>}
          </div>
          <div>
            <label className="block text-sm font-semibold mb-1 text-[#52616B]">Short Description</label>
            <textarea
              ...
              maxLength={150}
            />
            {errors.description && <span className="text-red-500 text-xs">Description required (min 10 characters)</span>}
          </div>
          <button
            type="submit"
        ...
        <div className="flex-1">
          <div className="bg-gradient-to-br from-[#faffee] to-[#e7f6d5] shadow-xl rounded-xl p-5 border border-[#caeb66]">
            <div className="flex items-center mb-3 gap-2">
              ...
              <span className="font-bold text-[#03373D] text-lg">Ad Preview</span>
            </div>
            <div className="border-b mb-2 border-[#dbecd6]" />
            <div>
              <div className="font-bold text-xl text-[#3c3c6e] mb-2 truncate">
                {adTitle ? adTitle : <span className="italic text-gray-400">Your ad title will appear here</span>}
              </div>
              <div className="text-sm text-[#3c3c6e] min-h-[40px]">
                {description ? description : <span className="italic text-gray-400">Description will be shown here</span>}
              </div>
              <div className="mt-3 text-xs text-gray-500">
                <span>Vendor: </span>
                <span className="font-medium">{user?.email || <span className="italic">Logged in vendor</span>}</span>
              </div>
            </div>
          </div>
          <div className="mt-2 text-xs text-gray-400 italic text-center">Your ad will be reviewed before publishing.</div>
        </div>
      </div>
    </div>
  );
