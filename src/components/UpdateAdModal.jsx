  ...
  const onSubmit = async (data) => {
    try {
      const res = await fetch(
        `https://bazar-bd-back-end-a12.onrender.com/advertisements/${ad._id}`,
        {
          method: "PUT",
  ...
  return (
    <div className='fixed inset-0 bg-opacity-40 flex items-center justify-center z-50'>
      <div className='bg-white p-8 rounded-3xl shadow-xl w-full max-w-lg'>
        <h3 className='text-2xl text-gray-900 font-semibold mb-6 text-center'>
          Update Advertisement
        </h3>
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
          <div className='relative'>
            <label htmlFor='adTitle' className=' left-4 top-0 text-gray-500'>
              Ad Title
            </label>
            <input
          ...
          <div className='relative'>
            <label
              htmlFor='description'
              className=' left-4 top-0 text-gray-500'
            >
              Description
            </label>
            <textarea
